...
dataset = mmPortraits(dataroot,
                      transform=transforms.Compose([                                                    transforms.Normalize((0.5,0.5,0.5),(0.5,0.5,0.5))]),
                                                              train=True, realValue = 1.0)
...
batchImg = real_batch['image'][:64,:,:,:]
grid = vutils.make_grid(batchImg, nrow=8, padding=2, normalize=True).cpu()
...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            #nn.convtranspose2d(in, out, kernel, stride, padding
...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(ndf, ndf * 2, 4, 2, 3, bias=False),
            nn.BatchNorm2d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(p=.4),
            # state size. (ndf*2) x 10 x 10
            nn.Conv2d(ndf * 2, ndf * 4, 4, 2, 3, bias=False),
            nn.BatchNorm2d(ndf * 4),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(p=.4),
            # state size. (ndf*4) x 7 x 7
            nn.Conv2d(ndf * 4, ndf * 8, 7, 2, 3, bias=False),
            nn.BatchNorm2d(ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Dropout2d(p=.4),
            # state size. (ndf*8) x 4 x 4
            nn.Flatten(),
            nn.Linear(4*4*ndf*8,1),
            nn.Sigmoid()
        )
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        if (iters % 500 == 0) or ((epoch == num_epochs-1) and (i == len(dataloader)-1)):
            ...
            if ((epoch==num_epochs-1) and (i==len(dataloader)-1)):
                plt.savefig("../results/no_img_add_final_" + str(epoch) + "_" + str(iters))
                plt.draw()
                plt.pause(0.001)
            else:
                plt.savefig("../results/no_img_add/" + str(epoch) + "_" + str(iters))
                plt.draw()
                plt.pause(0.001)
